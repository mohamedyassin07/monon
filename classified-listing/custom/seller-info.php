...
$FAL_license_number = get_user_meta($listing_post->post_author, '_im_FAL_license_number',true);
$store = false;
if (class_exists('RtclStore')) {
    $store = StoreFunctions::get_user_store($listing_post->post_author);
}
$user = get_user_by( 'id', $listing_post->post_author );
?>
<div class="classified-seller-info widget">
    ...
    <div class="rtin-box">
        ...
        <?php if ($listing->can_show_user()): ?>
            <div class="rtin-author">
                <?php if ($store): ?>
                    <?php $store->the_logo(); ?>
                    <h4 class="rtin-name">
                        <a href="<?php $store->the_permalink(); ?>"><?php $listing->the_author(); ?></a></h4>
                <?php else: ?>
        ...
        <?php if (Fns::registered_user_only('listing_seller_information') && !is_user_logged_in()) {
            ...
            ?>
            <p class="login-message"><?php echo wp_kses(sprintf(__("Please <a href='%s'>login</a> to view the seller information.", "classima"), esc_url($redirect_to)), ['a' => ['href' => []]]); ?></p>
        <?php } else { ?>

            <?php if ($address = Listing_Functions::get_single_contact_address($listing)): ?>
                <div class="rtin-location rtin-box-item clearfix" style="display: none;">
            ...
            <?php endif; ?>
            <?php if( ! empty($FAL_license_number) ): ?>
                <div class="rtin-store rtin-box-item clearfix">
                    <i class="fa fa-fw fa-id-card" aria-hidden="true"></i>
                    <span>رقم رخصة الفال  : <?php echo $FAL_license_number; ?></span>
                </div>
                <div class="rtin-store rtin-box-item clearfix">
                    <i class="fa fa-fw fa-user" aria-hidden="true"></i>
                    <span>  اسم الوسيط  : <?php echo $user->display_name; ?></span>
                </div>
            <?php endif; ?>
            <?php if ($store): ?>
                <div class="rtin-store rtin-box-item clearfix" style="display: none;">
